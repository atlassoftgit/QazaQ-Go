...
class AvailableOrderAdded extends MainEvent {
  Map<String, dynamic> order;

  AvailableOrderAdded(this.order);
}
      ...
      if (order.status == OrderStatus.started) {
        markers = order.points
            ...
            .map<MarkerData>((e) => MarkerData(
                ...
                position: LatLng(order.points[e.key].lat, order.points[e.key].lng),
...
class MainBloc extends Bloc<MainEvent, MainState> {
  MainBloc() : super(StatusOffline(null)) {
    ...
    on<AvailableOrderAdded>((event, emit) {
      if (state is StatusOnline) {
        if ((state as StatusOnline)
                .orders
                .indexWhere((element) => element.id == event.order['orderCreated']['id']) ==
            -1) {
          final AvailableOrderMixin created = AvailableOrders$Query$Order.fromJson(
    ...
    on<AvailableOrderRemoved>((event, emit) {
      if (state is StatusOnline) {
        final updated = OrderRemoved$Subscription.fromJson(event.order).orderRemoved;
        if ((state as StatusOnline).orders.indexWhere((element) => element.id == updated.id) > -1) {
          (state as StatusOnline).orders.removeWhere((element) => element.id == updated.id);
          emit(StatusOnline(
              driver: state.driver,
              orders: (state as StatusOnline).orders,
