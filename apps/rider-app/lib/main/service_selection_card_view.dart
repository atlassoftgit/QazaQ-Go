...
import 'package:ridy/wallet/payment_method_sheet_view.dart';
          ...
          options: QueryOptions(
              ...
              variables: GetFareArguments(
                      points: (state as OrderPreview)
                          .points
                          .map((e) => PointInput(lat: e.latlng.latitude, lng: e.latlng.longitude))
                          .toList(),
            ...
            if (result.hasException) {
              if (result.exception!.graphqlErrors
                ...
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  final snackBar = SnackBar(content: Text(S.of(context).error_region_unsupported));
                  ScaffoldMessenger.of(context).showSnackBar(snackBar);
                  mainBloc.add(ResetState());
                });
              }
              if (result.exception!.graphqlErrors.filter((element) => element.message == 'Coupon expired').isNotEmpty) {
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  final snackBar = SnackBar(content: Text(S.of(context).coupon_expired));
                  ScaffoldMessenger.of(context).showSnackBar(snackBar);
                  mainBloc.add(ResetState());
                });
              }
              return Center(
                child: Text(result.exception!.graphqlErrors.map((e) => e.message).join(',')),
              );
            }
              ...
              builder: (RunMutation runMutation, QueryResult? result) {
                return SelectServiceView(
                  ...
                  onServiceSelect: (String serviceId, int intervalMinutes) async {
                            ...
                            input: CreateOrderInput(
                                ...
                                optionIds: state.selectedOptions
                                    .filter((selectedOption) =>
                                        state.selectedService!.options.map((e) => e.id).contains(selectedOption))
                                    .toList(),
                                points: state.points
                                    .map((e) => PointInput(lat: e.latlng.latitude, lng: e.latlng.longitude))
                                    .toList(),
                                addresses: state.points.map((e) => e.address).toList()),
                            notificationPlayerId: fcmId ?? "")
                        .toJson();
                    final result = await runMutation(args).networkResult;
                    if (result!.hasException) {
                      if (result.exception!.graphqlErrors
                          .filter((element) => element.message == 'UNSUFFICIENT_CREDIT')
                          .isNotEmpty) {
                        var service = fareResult.services
                            .firstWhere((element) =>
                                element.services.indexWhere((element) => element.id == state.selectedService?.id) > -1)
                            .services
                            .firstWhere(
                              (element) => element.id == state.selectedService?.id,
                            );
                        ...
                        showModalBottomSheet(
                            ...
                            builder: (context) {
                              return PaymentMethodSheetView(
                                ...
                                amount: service.cost * service.prepayPercent / 100.0,
                              );
                            });
                        return;
                      }
                    ...
                    final order = CreateOrder$Mutation.fromJson(result.data!).createOrder;
  ...
  if (settings.authorizationStatus == AuthorizationStatus.denied) {
    showDialog(
        ...
        builder: (context) => AlertDialog(
              ...
              content: Text(S.of(context).message_notification_permission_denined_message),
              actions: [
                TextButton(
                  onPressed: () {
