...
export class SharedOrderService {
    ...
    async calculateFare(input: { points: Point[], twoWay?: boolean, coupon?: CouponEntity }) {
        ...
        const _cats = cats.map(cat => {
            ...
            const _services = services.filter(x => servicesInRegion.filter(y => y.id == x.id).length > 0).map(service => {
                ...
                if(input.coupon == null) {
                    ...
                    const costAfterCoupon = this.applyCouponOnPrice(input.coupon, cost);
                    return {
                        ...service,
                        cost,
                        costAfterCoupon
    ...
    async createOrder(input: { riderId: number, serviceId: number, intervalMinutes: number, points: Point[], addresses: string[], operatorId?: number, twoWay?: boolean, optionIds: string[] }): Promise<RequestEntity> {
        ...
        const options = await this.serviceOptionRepository.findByIds(input.optionIds);
        ...
        const optionFee = paidOptions.length == 0 ? 0 : paidOptions.map(option => option.additionalFee ?? 0).reduce((previous: number, current: number) => current += previous);
        const metrics = service.perHundredMeters > 0 ?
            await this.googleServices.getSumDistanceAndDuration(input.points) :
            { distance: 0, duration: 0 };
        const cost = this.servicesService.calculateCost(service, metrics.distance, metrics.duration) + optionFee;
        ...
        if ((input.intervalMinutes ?? 0) < 30) {
            ...
            Logger.log(`drivers with service: ${JSON.stringify(driversWithService)}`);
