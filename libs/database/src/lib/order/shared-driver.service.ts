...
export class SharedDriverService {
    ...
    async getOnlineDriversWithServiceId(driverIds: number[], serviceId: number, fleetIds: number[] = []) {
    ...
    async canDriverDoServiceAndFleet(driverId: number, serviceId: number, fleetIds: number[] = []): Promise<boolean> {
